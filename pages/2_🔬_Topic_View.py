...
from lib.taxonomy import build_taxonomy_lookups, get_domain_color
...
cfg = progressify(dom_df, ["% UL", "% Pubs LUE", "% PPtop10%", "% PPtop1%", "% internal collaboration", "% international",
...
# Lab FWCI whiskers (keep same order)
def _lab_q(col):
...
fr_copubs = [int(x) if x else 0 for x in _explode(drow["Top 20 FR partners (totals copubs in this domain)"])]
# Correct % column to use: “% of UL total copubs” for this partner (render as progress)
fr_pct_ul = [float(x) * 100.0 for x in _explode(drow["Top 20 FR partners (% of UL total copubs)"])]
...
int_copubs = [int(x) if x else 0 for x in _explode(drow["Top 20 int partners (totals copubs in this domain)"])]
int_pct_ul = [float(x) * 100.0 for x in _explode(drow["Top 20 int partners (% of UL total copubs)"])]
...
# Order fields canonically for this domain
